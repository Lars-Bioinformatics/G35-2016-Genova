...
rule bwa_mem:
    input:
        "fastq/{sampleid}_{protocol}_{flowcell}_R1.fastq.gz",
        "fastq/{sampleid}_{protocol}_{flowcell}_R2.fastq.gz"
    output:
        bam = "{sampleid}_{protocol}_{flowcell}.sorted.bam",
        bai = "{sampleid}_{protocol}_{flowcell}.sorted.bai"
...
rule qualimap_markdup:
    ...
    params:
        outdir = directory("MarkDuplicates/quality_control/{sample}_bamqc")
    resources: cpus=21, mem=30000
    shell:
        """
        qualimap bamqc \
        -bam {input} \
