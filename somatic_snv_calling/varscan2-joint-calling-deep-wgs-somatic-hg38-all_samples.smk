...
target_regions = "/work/sduvarcall/resources/hg38/wgs_chr_regions/"
...
INTERVALS=["chr"+x for x in [str(i) for i in range(1,23)]+["X","Y","M"]]
print(INTERVALS)
print (PATIENTS)
...
####                      Output                     ####
#########################################################
output_somatic = "varscan_somatic_joint_calling_default_all-samples/"
...
rule all_pairs:
    input:
        ...
        expand(output_somatic+"{sample}_cns_varscan2_1000g-pon_gnomad-filtered.vcf.gz", sample=PATIENTS),
        expand(output_somatic+"{sample}_cns_varscan2_1000g-pon_gnomadAF_above0_001-filtered.vcf.gz", sample=PATIENTS),
        # expand(output_somatic+"{sample}_cns_varscan2_1000g-pon-filtered.vcf.gz", sample=PATIENTS),
...
rule Varscan2_default:
    ...
    output:
        vcf=output_somatic+"split_vcf/{sample}_cns__{interval}__split_varscan2.vcf.gz",
        vcf_index=output_somatic+"split_vcf/{sample}_cns__{interval}__split_varscan2.vcf.gz.tbi"
    resources: cpus=1, mem=30000
    shell:
        """
        varscan -Xmx{resources.mem}g mpileup2cns {input.mpileups} \
        --output-vcf 1 \
        --variants 1 \
        --vcf-sample-list {input.sample_list} \
        | bgzip -c > {output.vcf}
...
rule pon_1000g_filtering:
    input:
        vcf = output_somatic+"{sample}_cns_varscan2.vcf.gz",
        pon = "/work/sduvarcall/resources/hg38/1000g_pon.hg38.vcf.gz"
    output:
        output_somatic+"{sample}_cns_varscan2_1000g-pon-filtered.vcf.gz"
    shell:
        """
        bcftools isec \
        -C \
...
rule gnomad_filtering:
    input:
        vcf = output_somatic+"{sample}_cns_varscan2_1000g-pon-filtered.vcf.gz",
        gnomad = "/work/sduvarcall/resources/hg38/af-only-gnomad.hg38.decomposed.filterAF_Above0_0001.vcf.gz"
    output:
        vcf = output_somatic+"{sample}_cns_varscan2_1000g-pon_gnomad-filtered.vcf.gz",
    shell:
        """
        bcftools isec \
        -C \
...
rule gnomad_filtering_0_001:
    input:
        vcf = output_somatic+"{sample}_cns_varscan2_1000g-pon-filtered.vcf.gz",
        gnomad = "/work/sduvarcall/resources/hg38/af-only-gnomad.hg38.decomposed.filterAF_Above0_001.vcf.gz"
    output:
        vcf = output_somatic+"{sample}_cns_varscan2_1000g-pon_gnomadAF_above0_001-filtered.vcf.gz",
    shell:
        """
        bcftools isec \
        -C \
