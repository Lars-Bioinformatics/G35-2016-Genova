...
####                       Input                     ####
#########################################################
project = "/work/sduvarcall/G35-2016-Genova/"
...
####                      Output                     ####
#########################################################
output_mutect2 = "/work/sduvarcall/G35-2016-Genova/MarkDuplicates/mutect2_ctDNA_pipeline/mutect2_matched/vcf_filterFlag_PASS_norm/"
...
rule all_pairs:
    input:
        ...
        #     normal=config[fam]["normal"],
        #     tumor=config[fam]["tumor"]) for fam in PAIR],
        [expand(output_mutect2+"maf_files/{tumor}_vs_{normal}_somatic_mutect2_filterFlag_norm_PASSonly.maf",
            normal=config[fam]["normal"],
            tumor=config[fam]["tumor"]) for fam in PAIR],
