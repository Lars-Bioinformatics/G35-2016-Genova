...
__author__ = "Jakob Jersild Nielsen <jjn@inguz.dev> and Lars Andersen <larsmew@gmail.com>"
__date__ = "2022/06/17"
__version__ = "1.2"
...
callable_regions = config["callable_regions"]
...
ALL_PLASMA = ["G35-P"+str(i)+"-PLASMA_thruplex-tag-seq-hv_HVFFCDSXY" for i in range(1,11)]
...
rule all:
    ...
    #        somaticseq=SOMATICSEQ) for i in MATCHED_BIOPSY_NORMAL.index],
        ...
        mrdetectsnv=[expand(output_dir+"variants_final/{ensemble}/{tumor}_vs_{normal}/MRDetectSNV_complete",
            ...
            ensemble=ENSEMBLE) for i in MATCHED_TUMOR_NORMAL.index],
