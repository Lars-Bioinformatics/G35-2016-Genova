...
'''
rule Mutect2_matched_calling_forced_plasmaPool:
    input:
        ...
        force_intervals=main_output+"union_vcf/{sample}"+force_intervals_suffix
    output:
        vcf="{output}plasmaPool/{sample}/vcf_files/{tumor}_vs_{normal}_somatic_mutect2-forced.vcf.gz",
        idx="{output}plasmaPool/{sample}/vcf_files/{tumor}_vs_{normal}_somatic_mutect2-forced.vcf.gz.tbi",
        realigned_bam="{output}plasmaPool/{sample}/realigned_bam/{tumor}_vs_{normal}_somatic_mutect2-forced.bam",
        vcf_stats="{output}plasmaPool/{sample}/vcf_files/{tumor}_vs_{normal}_somatic_mutect2-forced.vcf.gz.stats",
        f1r2="{output}plasmaPool/{sample}/f1r2/{tumor}_vs_{normal}_f1r2.tar.gz"
...
rule FilterMutectCalls_forced_plasmaPool:
    input:
...
rule vcf_norm_decompose_bcftools_plasmaPool:
    input:
        vcf="{output}vcf_filterFlag/{sample}_somatic_mutect2-forced_filterFlag.vcf.gz"
    output:
...
rule intersect_forcedVCF_mergedVCF_plasmaPool:
    input:
        norm_vcf="{output}plasmaPool/{sample}/vcf_filterFlag_norm/{tumor}_vs_{normal}_somatic_mutect2-forced_filterFlag_norm.vcf.gz",
        # force_intervals=lambda wildcards: expand(main_output+"union_vcf/{patient}"+force_intervals_suffix, patient=wildcards.normal[:5]+wildcards.normal[7:])
        force_intervals=main_output+"union_vcf/{sample}"+force_intervals_suffix
    output:
        final_vcf="{output}plasmaPool/{sample}/vcf_final/{tumor}_vs_{normal}__somatic_mutect2-forced_filterFlag_norm_final.vcf.gz"
    shell:
        """
