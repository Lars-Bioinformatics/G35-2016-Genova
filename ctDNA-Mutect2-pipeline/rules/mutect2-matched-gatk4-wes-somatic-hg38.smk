...
rule Mutect2_matched:
    input:
        normal="bam/{normal}"+bam_suffix,
        tumor="bam/{tumor}"+bam_suffix,
    output:
        vcf=mutect2_matched_output+"split/vcf_split/{tumor}_vs_{normal}_somatic_mutect2__{chr}.vcf.gz",
        idx=mutect2_matched_output+"split/vcf_split/{tumor}_vs_{normal}_somatic_mutect2__{chr}.vcf.gz.tbi",
        vcf_stats=mutect2_matched_output+"split/vcf_split/{tumor}_vs_{normal}_somatic_mutect2__{chr}.vcf.gz.stats",
        bam_subfile=mutect2_matched_output+"split/bam_split/{tumor}_vs_{normal}__{chr}_mutect2.bam",
        f1r2=mutect2_matched_output+"split/f1r2_split/{tumor}_vs_{normal}_f1r2__{chr}.tar.gz"
...
#     input:
#         vcf_subfile=expand(mutect2_matched_output+"split/vcf_split/{{tumor}}_vs_{{normal}}_somatic_mutect2__{chr}.vcf.gz", interval=INTERVALS)
#     output:
...
#     params:
#         vcf_subfile=expand("-I "+mutect2_matched_output+"split/vcf_split/{{tumor}}_vs_{{normal}}_somatic_mutect2__{chr}.vcf.gz", interval=INTERVALS)
#     shell:
#         """
...
#   input:
#         bam_subfile=expand(mutect2_matched_output+"split/bam_split/{{tumor}}_vs_{{normal}}__{chr}_mutect2.bam", interval=INTERVALS)
#   output:
#         bam=mutect2_matched_output+"realigned_bam/{tumor}_vs_{normal}_somatic_mutect2.bam"
#   params:
#         bam_subfile=expand("-I " + mutect2_matched_output + "split/bam_split/{{tumor}}_vs_{{normal}}__{chr}_mutect2.bam", interval=INTERVALS)
#   shell:
#         """
...
#     input:
#         vcf_subfile=expand(mutect2_matched_output+"split/vcf_split/{{tumor}}_vs_{{normal}}_somatic_mutect2__{chr}.vcf.gz.stats", interval=INTERVALS)
#     output:
#         vcf_stats=mutect2_matched_output+"vcf_files/{tumor}_vs_{normal}_somatic_mutect2.vcf.gz.stats",
#     params:
#         vcf_subfile=expand("-stats "+mutect2_matched_output+"split/vcf_split/{{tumor}}_vs_{{normal}}_somatic_mutect2__{chr}.vcf.gz.stats", interval=INTERVALS)
#     shell:
#         """
...
#     input:
#         f1r2=expand(mutect2_matched_output+"split/f1r2_split/{{tumor}}_vs_{{normal}}_f1r2__{chr}.tar.gz", interval=INTERVALS)
#     output:
#         f1r2_model=mutect2_matched_output+"read_orientation_models/{tumor}_vs_{normal}_read-orientation-model.tar.gz"
#     params:
#         f1r2=expand("-I "+mutect2_matched_output+"split/f1r2_split/{{tumor}}_vs_{{normal}}_f1r2__{chr}.tar.gz", interval=INTERVALS)
#     shell:
#         """
