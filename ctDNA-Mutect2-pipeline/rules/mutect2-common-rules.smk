rule merge_somatic_vcf:
    input:
        vcf_subfile=expand("{{output}}split/vcf_split/{{sample}}_somatic_mutect2__{chr}.vcf.gz", chr=CHROM)
    output:
        vcf="{output}vcf_files/{sample}_somatic_mutect2.vcf.gz",
        idx="{output}vcf_files/{sample}_somatic_mutect2.vcf.gz.tbi",
    params:
        vcf_subfile=expand("-I "+"{{output}}split/vcf_split/{{sample}}_somatic_mutect2__{chr}.vcf.gz", chr=CHROM)
    shell:
        """
...
rule merge_mutect2_bam:
  input:
        bam_subfile=expand("{{output}}split/bam_split/{{sample}}__{chr}_mutect2.bam", chr=CHROM)
  output:
        bam="{output}realigned_bam/{sample}_somatic_mutect2.bam"
  params:
        bam_subfile=expand("-I "+"{{output}}split/bam_split/{{sample}}__{chr}_mutect2.bam", chr=CHROM)
  shell:
        """
...
rule merge_somatic_vcf_stats:
    input:
        vcf_subfile=expand("{{output}}split/vcf_split/{{sample}}_somatic_mutect2__{chr}.vcf.gz.stats", chr=CHROM)
    output:
        vcf_stats="{output}vcf_files/{sample}_somatic_mutect2.vcf.gz.stats",
    params:
        vcf_subfile=expand("-stats "+"{{output}}split/vcf_split/{{sample}}_somatic_mutect2__{chr}.vcf.gz.stats", chr=CHROM)
    shell:
        """
...
rule learnReadOrientationModel:
    input:
        f1r2=expand("{{output}}split/f1r2_split/{{sample}}_f1r2__{chr}.tar.gz", chr=CHROM)
    output:
        f1r2_model="{output}read_orientation_models/{sample}_read-orientation-model.tar.gz"
    params:
        f1r2=expand("-I "+"{{output}}split/f1r2_split/{{sample}}_f1r2__{chr}.tar.gz", chr=CHROM)
    shell:
        """
