...
WORK="/work/sduvarcall/G35-2016-Genova/MarkDuplicates/"
INPUT_BAM = WORK+"bam"
OUTPUT = WORK+"cgpwgs_out"
...
bam_suffix = ".recalibrated"

ref_build = "GRCh38"

# Resources - paths inside docker
ressource_path = "/work/sduvarcall/G37-2016-BRCAX-Genomes/"
if ref_build == "GRCh37":
    # GRCh37 paths
    ref = ressource_path+"reference_files/core_ref_GRCh37d5"
    res = ressource_path+"reference_files"
else:
    # GRCh38 paths
    ref = ressource_path+"reference_files_GRCh38/core_ref_GRCh38_hla_decoy_ebv"
    res = ressource_path+"reference_files_GRCh38"
...
###############################################################################
#### If missing bam stats (bas) files
###############################################################################
rule bam_stats:
    input:
        bam=INPUT_BAM+"{sample}.bam"
    output:
        bas=INPUT_BAM+"{sample}.bam.bas"
    resources: cpus=12, mem_mb=72000
    shell:
        """
        udocker run \
        --user=pi \
...
rule rename_bam_index:
    input:
        bai=INPUT_BAM+"{sample}.bai"
    output:
        bai=INPUT_BAM+"{sample}.bam.bai"
    shell:
        """
        cp {input} {output}
        """
...
rule caveman_setup:
    input:
        normal=INPUT_BAM+"/{normal}"+bam_suffix+".bam",
        tumor=INPUT_BAM+"/{tumor}"+bam_suffix+".bam",
        n_bai=INPUT_BAM+"/{normal}"+bam_suffix+".bam.bai",
        t_bai=INPUT_BAM+"/{tumor}"+bam_suffix+".bam.bai"
    output:
...
rule BRASS_input:
    input:
        normal=INPUT_BAM+"/{normal}"+bam_suffix+".bam",
        tumor=INPUT_BAM+"/{tumor}"+bam_suffix+".bam",
        normal_bas=INPUT_BAM+"/{normal}"+bam_suffix+".bam.bas",
        tumor_bas=INPUT_BAM+"/{tumor}"+bam_suffix+".bam.bas"
    output:
    ...
    shell:
        ...
        --volume={res}:{res} \
        cgpwgs \
        bash -c '/usr/bin/time -v brass.pl -j 4 -k 4 -c {resources.cpus} \
...
rule BRASS_cover:
    ...
    shell:
        ...
        --volume={res}:{res} \
        cgpwgs \
        bash -c '/usr/bin/time -v nice -n 10 brass.pl -j 4 -k 4 -c {resources.cpus} \
        -d {res}/brass/HiDepth.bed.gz \
        -f {res}/brass/brass_np.groups.gz \
        -g {ref}/genome.fa \
        -s {SPECIES} -as {ASSEMBLY} -pr {PROTOCOL} -pl ILLUMINA \
...
rule caveman:
    ...
    resources: cpus=42, mem_mb=126000
    shell:
        """
